...
namespace YOlshinAnton.QueryBuilder
{
    class Program
    {
        ...
        {
            // 0 - AlertId, 11 - PersonId, 13 - LegalId
            var columns = new List<int>() { 0, 11, 13 };

            var query = QueryBuilder.BuildQuery(columns);
...
namespace YOlshinAnton.QueryBuilder
{
    ...
    public static class Database
    {
        ...
        public class TableColumn : IComparable
        {
            ...
            public int CompareTo(object obj)
            {
                return TableId.CompareTo(((TableColumn)obj).TableId);
            }
        }
        ...
        public static class Repository
        {
            ...
            /// REQUIRED
            /// </summary>
            public static Func<GraphEdge[]> SelectDatabaseGraphEdges = GetDatabaseGraphEdges;
            ...
            public static TableColumn[] GetTableColumns(int tableId)
            {
                var all = GetTableColumns();
                var saticfied = new List<TableColumn>();

                foreach (var column in all) if (column.TableId == tableId)

                        saticfied.Add(column);

                return saticfied.ToArray();
            }
            ...
            public static GraphEdge[] GetDatabaseGraphEdges()
            {
                return new GraphEdge[]
                {
                new GraphEdge() { Id=0, Table1=0, Table2=1, JoinCondition="left join Mission M on A.Id = M.AlertId" },
                new GraphEdge() { Id=1, Table1=0, Table2=2, JoinCondition="left join Document D on A.Id = D.AlertId" },
                new GraphEdge() { Id=2, Table1=0, Table2=3, JoinCondition="left join Loan L on A.Id = L.AlertId" },
                new GraphEdge() { Id=3, Table1=0, Table2=4, JoinCondition="left join Entity E on A.Id = E.ALertId" },
                new GraphEdge() { Id=4, Table1=4, Table2=5, JoinCondition="left join EntitySubType Est on E.EntitySubTypeId = Est.Id" },
                new GraphEdge() { Id=5, Table1=5, Table2=6, JoinCondition="left join EntityType Et on Est.EntityTypeId = Et.Id" },
                new GraphEdge() { Id=6, Table1=4, Table2=7, JoinCondition="left join Negative Neg on E.Id = Neg.EntityId" },
                new GraphEdge() { Id=7, Table1=4, Table2=8, JoinCondition="left join Person P on E.ObjectId = P.Id" },
                new GraphEdge() { Id=8, Table1=4, Table2=9, JoinCondition="left join Legal Lgl on E.ObjectId = Lgl.Id" }
                };
            }
            public static int[,] GetDatabaseGrahpAdjacent(GraphEdge[] edges)
            {
    ...
    public static class GraphAlgorithms
    {
        ...
        public static Vertex<T> WidthSearch<T>(Graph<T> G, T A, T B)
        {
            ...
            for (int i = 0; i < G.VertexCount; ++i)
                ...
                if (A.Equals(G.Vertexes[i])) stack.Push(new Vertex<T>() { Index = i, Value = G.Vertexes[i] });
            ...
            while (stack.Count != 0)
            {
                ...
                for (int i = 0; i < G.VertexCount; ++i)
                {
                    ...
                    if (vertexIsAdjacentAndNotVisited)
                    {
                        ...
                        stack.Push(new Vertex<T>() { Index = i, Value = vertex });
    ...
    /// 
    /// 3   Удовлетворить сответствие названных справочников классам их представляющих:
    ///     Table,
    ///     TableColumn,
    ///     DatabaseGraphEdges;
    ///     
    ...
    public static class QueryBuilder
    {
        ...
        {
            if (columnIds == null || columnIds.Count == 0) throw new ArgumentException("columnIds is empty");
        ...
        public static string BuildSelect(Database.TableTableColumn[] tablesTableColumns)
        {
        ...
        static private string EdgesToJoins(List<Edge<int>> routeEdges, Database.GraphEdge[] graphEdges)
        {
        ...
        public static string BuildFrom(List<int> queryTableIndexes, Graph<Database.Table> G, Database.GraphEdge[] graphEdges)
        {
            // Искомый путь
